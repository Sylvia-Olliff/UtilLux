...
public static class Program
{
    ...
    private static ExitCode Run(string[] args)
    {
        ...
        try
        {
            if (args.Length != 0)
            {
                return ExitCode.UtilLuxNotRunning;
            }
            ...
            logger.LogInformation("UtilLux service execution started");

            host.Start();
            mainLoopRunner.RunMainLoop(token: applicationLifetime.ApplicationStopping);
            host.WaitForShutdown();

            logger.LogInformation("UtilLux service execution stopped");
        }
    ...
    private static void ConfigureServices(HostBuilderContext context, IServiceCollection services) =>
        services.AddHostedService<Worker>()
            .Configure<HostOptions>(opts => opts.ShutdownTimeout = TimeSpan.FromMilliseconds(100))
    ...
    private static Mutex ConfigureSingleInstance(IServiceProvider services) =>
        services
            ...
            .TryAcquireMutex(nameof(UtilLux));
    ...
    private static void SubscribeToExternalCommands(IHost host, ILogger logger)
    {
        ...
        namedPipeService.StartServer(nameof(UtilLux));
    ...
    {
        var processes = Process.GetProcessesByName(nameof(UtilLux));
        bool isRunning = processes.Length > 1;

        Console.WriteLine(isRunning ? "UtilLux is running" : "UtilLux is not running");

        return isRunning ? ExitCode.Success : ExitCode.UtilLuxNotRunning;
    }
